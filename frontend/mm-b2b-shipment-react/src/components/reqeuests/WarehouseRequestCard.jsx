    ...
    return (
        <Card size={'sm'} style={{ display: 'flex', flexDirection: 'column', height: '100%' }} variant='outline'>
            ...
            <Stack spacing={0} style={{ flex: 1, display: 'flex', flexDirection: 'column', justifyContent: 'space-between' }}>
                ...
                <Divider/>
                <CardFooter>
                    <AcceptButton changeStatus={() => updateStorageStatus(id, { ...warehouse, requestStatus: 'APPROVED' })} fetchEntity={fetchWarehousesForStorage}/>
                    <DeclineButton changeStatus={() => updateStorageStatus(id, { ...warehouse, requestStatus: 'REJECTED' })} fetchEntity={fetchWarehousesForStorage}/>
                </CardFooter>
            </Stack>
        </Card>
    );
